...

{
   ...
   */
  pr_err("SELinux:  Runtime disable is deprecated, use selinux=0 on the kernel "
         "cmdline.\n");
...
static int sel_make_bools(struct selinux_policy *newpolicy,
                          struct dentry *bool_dir, unsigned int *bool_num,
                          char ***bool_pending_names,
                          unsigned int **bool_pending_values);
...
static ssize_t sel_read_mls(struct file *filp, char __user *buf, size_t count,
  ...
  length = scnprintf(tmpbuf, TMPBUFLEN, "%d", security_mls_enabled(fsi->state));
  return simple_read_from_buffer(buf, count, ppos, tmpbuf, length);
}
...
static int sel_open_policy(struct inode *inode, struct file *filp) {
  ...
  rc = avc_has_perm(&selinux_state, current_sid(), SECINITSID_SECURITY,
...
static ssize_t sel_read_policy(struct file *filp, char __user *buf,
  ...
  ret = avc_has_perm(&selinux_state, current_sid(), SECINITSID_SECURITY,
                     SECCLASS_SECURITY, SECURITY__READ_POLICY, NULL);
  if (ret)
    return ret;
...
static int sel_make_policy_nodes(struct selinux_fs_info *fsi,
  ...
  unsigned long tmp_ino =
      fsi->last_ino; /* Don't increment last_ino in this function */

  tmp_parent = sel_make_disconnected_dir(fsi->sb, &tmp_ino);
  if (IS_ERR(tmp_parent))
    return PTR_ERR(tmp_parent);

  tmp_ino = fsi->bool_dir->d_inode->i_ino -
  ...
  tmp_ino = fsi->class_dir->d_inode->i_ino -
            1; /* sel_make_dir will increment and set */
  ...
  ret = sel_make_bools(newpolicy, tmp_bool_dir, &tmp_bool_num, &tmp_bool_names,
                       &tmp_bool_values);
  if (ret) {
  ...
  ret = sel_make_classes(newpolicy, tmp_class_dir, &fsi->last_class_ino);
  if (ret) {
...
static ssize_t sel_read_checkreqprot(struct file *filp, char __user *buf,
  ...
  length = scnprintf(tmpbuf, TMPBUFLEN, "%u", checkreqprot_get(fsi->state));
  return simple_read_from_buffer(buf, count, ppos, tmpbuf, length);
}
...
static ssize_t sel_write_checkreqprot(struct file *file, const char __user *buf,
  ...
  length = avc_has_perm(&selinux_state, current_sid(), SECINITSID_SECURITY,
                        SECCLASS_SECURITY, SECURITY__SETCHECKREQPROT, NULL);
  if (length)
    return length;
  ...
  if (new_value) {
    ...
    pr_warn_once("SELinux: %s (%d) set checkreqprot to 1. This is deprecated "
                 "and will be rejected in a future kernel release.\n",
                 comm, current->pid);
  }
...
static ssize_t sel_write_create(struct file *file, char *buf, size_t size) {
  ...
  length = avc_has_perm(&selinux_state, current_sid(), SECINITSID_SECURITY,
                        SECCLASS_SECURITY, SECURITY__COMPUTE_CREATE, NULL);
  if (length)
    goto out;
  ...
  length =
      security_transition_sid_user(state, ssid, tsid, tclass, objname, &newsid);
  if (length)
    goto out;
...
static ssize_t sel_write_relabel(struct file *file, char *buf, size_t size) {
  ...
  length = avc_has_perm(&selinux_state, current_sid(), SECINITSID_SECURITY,
                        SECCLASS_SECURITY, SECURITY__COMPUTE_RELABEL, NULL);
  if (length)
    goto out;
...
static ssize_t sel_write_member(struct file *file, char *buf, size_t size) {
  ...
  length = avc_has_perm(&selinux_state, current_sid(), SECINITSID_SECURITY,
                        SECCLASS_SECURITY, SECURITY__COMPUTE_MEMBER, NULL);
  if (length)
    goto out;
...
static ssize_t sel_read_bool(struct file *filep, char __user *buf, size_t count,
  ...
  ret = -EINVAL;
  if (index >= fsi->bool_num || strcmp(name, fsi->bool_pending_names[index]))
    goto out_unlock;
...
static ssize_t sel_write_bool(struct file *filep, const char __user *buf,
  ...
  length = avc_has_perm(&selinux_state, current_sid(), SECINITSID_SECURITY,
                        SECCLASS_SECURITY, SECURITY__SETBOOL, NULL);
  if (length)
    goto out;
...
static ssize_t sel_commit_bools_write(struct file *filep,
  ...
  if (new_value && fsi->bool_pending_values)
    length =
        security_set_bools(fsi->state, fsi->bool_num, fsi->bool_pending_values);
...
static int sel_make_bools(struct selinux_policy *newpolicy,
  ...
  for (i = 0; i < num; i++) {
    ...
    ret = selinux_policy_genfs_sid(newpolicy, "selinuxfs", page, SECCLASS_FILE,
                                   &sid);
    if (ret) {
      pr_warn_ratelimited(
...
static ssize_t sel_read_avc_cache_threshold(struct file *filp, char __user *buf,
  ...
  length =
      scnprintf(tmpbuf, TMPBUFLEN, "%u", avc_get_cache_threshold(state->avc));
  return simple_read_from_buffer(buf, count, ppos, tmpbuf, length);
}
...

{
  ...
  ret = avc_has_perm(&selinux_state, current_sid(), SECINITSID_SECURITY,
                     SECCLASS_SECURITY, SECURITY__SETSECPARAM, NULL);
  if (ret)
    return ret;
...
static int sel_avc_stats_seq_show(struct seq_file *seq, void *v) {
  ...
  if (v == SEQ_START_TOKEN) {
    seq_puts(seq, "lookups hits misses allocations reclaims frees\n");
  } else {
...
static int sel_make_perm_files(struct selinux_policy *newpolicy, char *objclass,
                               int classvalue, struct dentry *dir) {
  int i, rc, nperms;
  char **perms;
...
static int sel_make_policycap(struct selinux_fs_info *fsi) {
  ...
  for (iter = 0; iter <= POLICYDB_CAPABILITY_MAX; iter++) {
    if (iter < ARRAY_SIZE(selinux_policycap_names))
      dentry = d_alloc_name(fsi->policycap_dir, selinux_policycap_names[iter]);
    else
...
static int sel_fill_super(struct super_block *sb, struct fs_context *fc) {
  ...
  fsi->policycap_dir =
      sel_make_dir(sb->s_root, POLICYCAP_DIR_NAME, &fsi->last_ino);
...
static int __init init_sel_fs(void) {
  struct qstr null_name = QSTR_INIT(NULL_FILE_NAME, sizeof(NULL_FILE_NAME) - 1);
  int err;
  ...
  selinux_null.dentry =
      d_hash_and_lookup(selinux_null.mnt->mnt_root, &null_name);
